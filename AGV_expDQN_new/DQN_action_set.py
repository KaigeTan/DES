...
load_dataB_new = sio.loadmat('B_new.mat')
...
# B = load_dataB['B']   #0 ~ 55
B_new = load_dataB_new['B_new']  # in the modified action sets, we use B_new
...
#cwd = os.getcwd()
cwd = "C:\\work\\DES_RL\\AGV_expDQN_new\\results1014\\"
...
for num_episode in range(NUM_EPISODE):
    ...
    while True:         
        ...
        [S_, all_S_, R, isDone, IfAppear32, stop_ind, selected_action] = \
            StepFun(S, A, AGV_1, AGV_2, AGV_3, AGV_4, AGV_5, SUP_IPSR, SUP_ZWSR, E_c, B_new, E)
        ...
        S = S_
        S_norm = S_norm_
        if isDone or episode_step > 300:
            if stop_ind == 1:
                stop_reason = 'pattern is impossible'
            elif stop_ind == 1:
                stop_reason = 'bad event pattern'
            elif episode_step > 300:
                stop_reason = 'reach 300 steps'
            else:
...
[action_list, pattern_list, state_list_his] = RL.check_action(500, S, check_pt_path, AGV_1, AGV_2, AGV_3, AGV_4, AGV_5, SUP_IPSR, SUP_ZWSR, E_c, E_u, B_new)
